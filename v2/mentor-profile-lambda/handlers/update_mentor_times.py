import json
from utils.db import get_mentor, update_available_times
from utils.auth import get_user_id_from_token
from utils.response import success_response, error_response
from utils.validators import validate_availability_times

def handle_update_mentor_times(event: dict) -> dict:
    """멘토 상담 가능 시간 수정 핸들러"""
    try:
        # 인증 확인 - 멘토 본인만 수정 가능
        ...
        if not available_times:
            return error_response(400, 'MISSING_FIELDS', 'available_times is required')

        # 상담 가능 시간 검증
        valid, msg = validate_availability_times(available_times)
        if not valid:
            return error_response(400, 'INVALID_TIMES', msg)
        ...
        # 멘토 상담 가능 시간 수정
        updated_mentor = update_available_times(mentor_id, available_times)
        ...
        return success_response(200, mentor_data, 'Mentor times updated successfully')

    except Exception as e:
        print(f"Update mentor times error: {str(e)}")

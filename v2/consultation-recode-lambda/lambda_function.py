import json
from handlers.create_record import handle_create_record
from handlers.get_record import handle_get_record
from handlers.get_all_records import handle_get_all_records
...
def lambda_handler(event, context):
    ...
    try:
        # POST /records
        if path == '/records' and http_method == 'POST':
            return handle_create_record(event)

        # GET /records  (전체 목록 - 단건보다 먼저 체크)
        elif path == '/records' and http_method == 'GET':
            return handle_get_all_records(event)

        # GET /records/{request_id}
        elif path and path.startswith('/records/') and http_method == 'GET':
            return handle_get_record(event)
